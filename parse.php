			...
			if($matche[1]){
				...
				do{
					...
					foreach($temp as $val) switch($preg[0]){ 
						case '>': if($el[$val][5]) $temp2 = array_merge($temp2, $el[$val][5]); break; 
						case '<': if(!in_array($el[$val][4],$temp2))$temp2[] = $el[$val][4];  break;
...
//I believe the rest of the page can be safely deleted.
